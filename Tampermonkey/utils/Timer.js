...
class Timer {
    ...
    #sleep(ms) {
        return new Promise((res) => ms <= 0 ? res() : setTimeout(res, ms));
    }
    ...
    #syncModePresets = {
        ...
        allowLostSync() {
            ...
            this.#sleep(this.#internal.duration).then(() => {
                ...
                this.#internal.offset = (new Date).getTime() - this.#internal.lastTime - this.#internal.duration;
                // 计算丢帧数
                let lostSyncTimes = parseInt(this.#internal.offset / this.timeout);
                // 警告信息
                if (this.debug && lostSyncTimes > 0) {
                    console.warn(`Lost sync: ${lostSyncTimes} times!`);
                }
                // 丢帧
...
/* ##### Test Case ##### */
let t = new Timer();
t.timeout = 1000 / 60; // 模拟一秒60帧
t.handler = () => console.log("渲染一帧");
t.start()

let sec = new Timer();
sec.timeout = 1000;
sec.handler = () => console.log("一秒过去了...")
sec.start()
