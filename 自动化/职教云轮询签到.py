...
def zdqd(userName, userPwd):
    ...
    login = session.post(url_api + "MobileLogin/newSignIn",
                         data=userData, headers=headers)
    ...
    todayClassData = {
        "stuId": loginInfo["userId"], "faceDate": faceDate, "newToken": loginInfo["newToken"]}
    todayClassData.update(basicData)
    # 签到列表
    todayClass = session.post(
        url_api + "faceteach/getStuFaceTeachList", data=todayClassData, headers=headers)
    ...
    if len(todayClassInfo) == 0:
        ...
        for i in range(len(todayClassInfo)):
            ...
            inClass = session.post(
                url_api + "faceteach/newGetStuFaceActivityList", data=inClassData, headers=headers)
            ...
            for n in range(len(inClassInfo)):
                if (inClassInfo[n]["DataType"] == "签到" and inClassInfo[n]["State"] == 2):
                    ...
                    attend = session.post(
                        url_api + "faceteach/isJoinActivities", data=attendData, headers=headers)
                    ...
                    if attendInfo["isAttend"] != 1:
                        ...
                        signIn = session.post(
                            url_api + "faceteach/saveStuSignNew", data=signInData, headers=headers)
                        signInInfo = json.loads(signIn.text)
                        signInTime = time.strftime(
